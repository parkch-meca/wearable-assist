...
# reuse trunk from palm50 (shoulder over table) if available
import os.path
tp=json.load(open('/tmp/cmp_render/palm50_pose.json'))['pose'] if os.path.exists('/tmp/cmp_render/palm50_pose.json') else None
tpose={c:0.0 for c in names}
for k in ['hip_flexion_r','hip_flexion_l','knee_angle_r','knee_angle_l','ankle_angle_r','ankle_angle_l','pelvis_tilt','pelvis_tx','pelvis_ty']+LUMB:
    if tp and k in tp: tpose[k]=tp[k]
setc(tpose); realize()
print("trunk shoulderR",np.round(Bd('humerus_R'),2))
def solve_arm(side):
    sgn=+1 if side=='R' else -1
    palmT=np.array([BCX, TOPb+0.17, sgn*HALF])    # palm center on upper side face
    tipT =np.array([BCX, TOPb+0.02, sgn*HALF])    # fingertip on lower side face (=> fingers wrap down)
