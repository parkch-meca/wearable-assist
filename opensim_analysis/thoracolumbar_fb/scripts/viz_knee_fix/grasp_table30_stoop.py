...
lb=np.array([20,-34,-32,-62,-45,-34]); ub=np.array([125,0,8,0,0,0])  # knee small(stoop), pelvis/lumbar/thoracic free to flex
SH_TGT=TOPb+GRIP_H+0.46   # reachable shoulder height via stoop
def tobj(x):
    x=np.clip(x,lb,ub); p=ground(bp(x)); sh=Bd('humerus_R'); pel=Bd('pelvis')
    reach=abs(sh[1]-SH_TGT)*1.4                    # drop shoulder to reachable height (via STOOP)
    knee_deep=max(0,(-x[1])-26)*0.9                # STOOP: keep knee small (penalize deep bend > ~26deg)
    head_up=max(0,(sh[1]-headY())+0.02)*2.0        # head not buried (still look toward box)
    ...
    # NOTE: no back-lean penalty -> a stooped (bent) back is the NATURAL posture for a knee-height box
    return 3*reach+knee_deep+head_up+10*abs(toe-heel)+8*(max(heel,toe)-FLOOR)+5*bal+0.5*pen
tb=None
for s_ in range(60):
