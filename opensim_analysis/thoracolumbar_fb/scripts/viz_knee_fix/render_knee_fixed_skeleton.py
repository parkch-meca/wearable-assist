"""Visualization-only knee fix (v2): place each bone MESH at its OpenSim socket-frame
ground transform. .osim untouched. Exact OpenSim default pose. Renders opaque skeleton."""
...
RES_X,RES_Y=760,1150

def mat_from(R,p):
    return mathutils.Matrix(((R[0],R[1],R[2],p[0]),(R[3],R[4],R[5],p[1]),(R[6],R[7],R[8],p[2]),(0,0,0,1)))
def look_at(cl,t):
    cl=mathutils.Vector(cl); t=mathutils.Vector(t)
    f=(t-cl).normalized(); up=mathutils.Vector((0,1,0))
    r=f.cross(up); r=r if r.length>1e-6 else mathutils.Vector((1,0,0)); r.normalize()
...
def white_mat():
    ...
    if "Emission Strength" in b.inputs: b.inputs["Emission Strength"].default_value=0.35
    return m
...
# --- FIX: each bone mesh -> OpenSim socket-frame ground transform ---
wm=white_mat(); fixed=0
for o in list(bpy.data.objects):
    if o.type=='MESH' and o.get('MuSkeMo_type')=='GEOMETRY':
        if o.name in MX:
            o.parent=None
            o.matrix_world=mat_from(MX[o.name]['R'],MX[o.name]['p'])
            fixed+=1
        o.hide_render=False; o.data.materials.clear(); o.data.materials.append(wm)
    elif o.type=='CURVE' or o.get('MuSkeMo_type') in ('WRAP','LANDMARK','JOINT'):
        o.hide_render=True
for coll in bpy.data.collections:
    if any(k in coll.name.lower() for k in ['wrap','joint cent','landmark','frame','muscle']):
        coll.hide_render=True
bpy.context.view_layer.update()
print("[fix] meshes placed:",fixed)
def span(nm):
    o=bpy.data.objects.get(nm); ys=[(o.matrix_world@mathutils.Vector(v))[1] for v in o.bound_box]
    return min(ys),max(ys)
for nm in ['skull.vtp','femur.vtp','tibia.vtp','foot.vtp']:
    lo,hi=span(nm); print(f"  {nm}: {lo:.3f}..{hi:.3f}")
...
VIEWS={"side":(cx,cy,cz+5),"back":(cx-5,cy,cz),"oblique":(cx+3.5,cy+0.7,cz+3.5),"front":(cx+5,cy,cz)}
...
for vn,loc in VIEWS.items():
    ...
    fp=os.path.join(OUT,f"TLFB_fixed_{vn}.png"); sc.render.filepath=fp
    bpy.ops.render.render(write_still=True); print("[render]",vn)
print("KNEE_FIXED_V2_DONE")
