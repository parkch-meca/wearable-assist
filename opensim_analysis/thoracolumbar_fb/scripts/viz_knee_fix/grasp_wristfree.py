...
json.dump({'pose':{k:float(v) for k,v in pose.items()},'geo':{'edge':EDGE,'top':TOPb,'grip':[BCX,TOPb+0.12,0.0],'half':HALF},'resid':{k:float(v) for k,v in g.items()}}, open('/tmp/cmp_render/wf_pose.json','w'))
print("SAVED wf_pose.json")
