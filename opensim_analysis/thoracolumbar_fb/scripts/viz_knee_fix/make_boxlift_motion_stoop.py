...
def kp(t):
    ...
    if t<2.0: a=sm((t-0.5)/1.5); return {c:(1-a)*P_stand[c]+a*P_grasp[c] for c in coords}   # reach+grasp
    if t<2.4: return P_grasp                                                                  # grasp hold
    if t<4.0: a=sm((t-2.4)/1.6); return {c:(1-a)*P_grasp[c]+a*P_carry[c] for c in coords}      # STAND UP: box tracks hands table->waist (continuous lift)
    if t<4.6: return P_carry                                                                   # carry hold
    if t<6.0: a=sm((t-4.6)/1.4); return {c:(1-a)*P_carry[c]+a*P_stand[c] for c in coords}       # return
    return P_stand
rows=[]
for t in ts:
