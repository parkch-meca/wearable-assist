...
lb=np.array([20,-125,-32,-40,-38,-33]); ub=np.array([125,0,8,0,0,0])
SH_TGT=TOPb+GRIP_H+0.42   # target shoulder height so arm (~0.56) reaches grip
def tobj(x):
    x=np.clip(x,lb,ub); p=ground(bp(x)); sh=Bd('humerus_R')
    reach=abs(sh[1]-SH_TGT)*1.4                    # stronger pull to reachable height
    ba=back_angle(); lean=max(0,ba-55)*1.2        # keep visual back < ~55 deg (NOT horizontal)
    fwd=max(0,sh[0]-0.16)                          # shoulder not dumped too far forward
    ...
    return 3*reach+lean+2*fwd+4*head_up+10*abs(toe-heel)+8*(max(heel,toe)-FLOOR)+6*bal+0.5*pen
tb=None
for s_ in range(60):
...
alb=np.array([0,-90,-90,0,-90,-70,-25]); aub=np.array([155,155,45,150,90,70,35])
...
ab=None
for s_ in range(40):
    rr=minimize(aobj,alb+(aub-alb)*np.random.RandomState(s_).rand(7),method='Nelder-Mead',options={'maxiter':2500})
...
json.dump(pose, open('/tmp/cmp_render/dist_pose.json','w'))
...
json.dump({'mesh':mesh_x,'sides':sides,'muscles':mus,
          ...
          open('/tmp/cmp_render/dist_frame.json','w'))
