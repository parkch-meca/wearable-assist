"""[인계] docs/HANDOVER_multijoint.md 생성.

수치는 전부 paper_numbers(단일 소스)에서 가져온다. 하드코딩 금지.
경로는 실제 존재를 확인한 것만 기재한다.
"""
import os
import sys
import json
...
ROOT = '/data/wearable-assist'
DOCS = f'{ROOT}/opensim_analysis/thoracolumbar_fb/docs'
OUT = f'{DOCS}/HANDOVER_multijoint.md'
MODELS = '/data/opensim_models/ThoracolumbarFB/Fullbody_TLModels_v2.0_OS4x'
ROMMODEL = f'{MODELS}/MaleFullBodyModel_v2.0_OS4_modified_no_coupler_M1scap_armfix_rom.osim'

MOTS = {'squat': '/data/stoop_motion/squat_synthetic_v1.mot',
        'stoop': '/data/stoop_results/stoop_v5/v5_30fps_armfix.mot',
        'box': '/data/stoop_motion/box_stoop_lift_m1_armfix.mot',
        'gait': '/data/gait_motion/gait_retarget_so.mot',
        'carry': '/data/gait_motion/carry_walk_so_armfix.mot'}
...
def row(p, note):
    ...
    if not e:
        return None
    if os.path.isdir(e):
        size = f"{len(os.listdir(e))}개 항목"
    else:
        n = os.path.getsize(e)
        size = f'{n/1e6:.1f} MB' if n >= 1e6 else f'{n/1e3:.0f} KB'
    return f'| `{p}` | {size} | {note} |'
...
IL = pn.gait_group('Iliocostalis (IL)')
LTpL = pn.gait_group('Longissimus pars lumborum (LTpL)')

# 모델 사실은 실제 모델에서 읽는다
import opensim as osim
_m = osim.Model(ROMMODEL)
_s = _m.initSystem()
N_COORD = _m.getCoordinateSet().getSize()
N_MUS = _m.getMuscles().getSize()
MASS = _m.getTotalMass(_s)
BASE_HASH = sha12(ROMMODEL)
RUN_HASH = sha12('/data/romfix_unified/squat_off/model_res_tight.osim')
RES_MAX = max(pn.reserve(k) for k in pn.ORDER)

order_by_effect = sorted([k for k in pn.ORDER if k != 'gait'],
                         key=lambda k: B[k]['rel'])

md = f"""# 인계 문서 — 복합관절(허리+어깨+팔꿈치) 슈트 신규 연구
...
| 기저 모델 해시 (SHA-1 앞 12자리) | `{BASE_HASH}` |
| 해석 실행 모델 해시 (reserve 포함, 5동작 10회 전부 동일) | `{RUN_HASH}` |
| 좌표 수 | {N_COORD} |
| 근육 수 | {N_MUS} (ES 76) |
| 총 질량 | {MASS:.6f} kg |
| reserve | tight (척추 `_FE`/`_LB`/`_AR`/`Abs_` optimal_force 5 N·m) |
| 척추 reserve 최대 (전 동작) | {RES_MAX:.2f} N·m |
...
|---|---|---:|---:|---:|---:|
""" + '\n'.join(
    f"| {pn.NAME[k]} | {pn.LOAD[k]} | {B[k]['off_s']} | {B[k]['on_s']} | "
    f"{B[k]['dpp_s']} | **{B[k]['rel_s']}** |" for k in pn.ORDER) + f"""
...
|---|---:|---:|---:|
""" + '\n'.join(
    f"| {pn.NAME[k]} | {A[k]['rel_s']} % | **{B[k]['rel_s']} %** | {C[k]['rel_s']} % |"
    for k in pn.ORDER) + f"""

### 1.3 핵심 서사

1. **하중 의존성** — 20 kg 두 동작({pn.NAME['box']} {B['box']['rel_s']} %,
   {pn.NAME['carry']} {B['carry']['rel_s']} %)이
   {abs(B['box']['rel'] - B['carry']['rel']):.1f} %p 이내로 수렴하고,
   0 kg 두 동작({B['squat']['rel_s']} %, {B['stoop']['rel_s']} %)과 뚜렷이 갈린다.
   부하가 상대 효과를 가르는 주 변수다.
2. **보행은 감소가 아니라 재분배** — 주 지표가 {B['gait']['rel_s']} %로 **증가**하는 반면
   (c) ES mean은 {C['gait']['rel_s']} %로 감소한다. 근육군으로 분해하면
   장늑근(IL) {IL['rel_s']} %, 최장근 요추부(LTpL) {LTpL['rel_s']} %.
   총량은 {pn.fmt(pn.GAIT_TOTAL_DPP, 2, True)} %p 감소하나 최대 활성 근육으로 집중된다.
...
- **포화** — {pn.NAME['box']}·{pn.NAME['carry']} 모두 슈트 OFF 정점이 상한
  ({A['box']['off_s']} %, {A['carry']['off_s']} %)에 도달한다. 두 동작의 효과는 **하한 추정**이다.
...
| 논문 | 확인할 것 |
|---|---|
| Hasenmaier et al. 2026 (`doi:10.3389/fbioe.2026.1631785`) | MES 정규화 기준이 본 연구 ES peak와 대응하는지. 본 연구 스툽 baseline({A['stoop']['off_s']} %)과 실측 69.8 %MVC의 근접성이 핵심 근거이므로 결정적 |
...
{table([
    row(f'{DOCS}/five_motion_paper.pdf', '최종본 (20쪽, A4)'),
    row(f'{DOCS}/five_motion_paper.docx', '편집용'),
    row(f'{DOCS}/five_motion_paper_draft.md', '초안 (국문)'),
    row(f'{DOCS}/five_motion_completion_record.md', '완결 기록 §1–§11'),
    row(f'{DOCS}/shoulder_dof_diagnosis.md', '좌측 어깨 진단 보고서'),
])}

### 10.2 발표자료

{table([
    row('/data/opensim_results/SMA_suit_5motion_presentation.pptx',
        '33슬라이드, 영상 5종 임베드. ⚠️ 리포지토리 밖 (.gitignore)'),
    row('/data/opensim_results/SMA_suit_5motion_presentation.pdf', 'pdf 백업'),
    row('/data/opensim_results/ppt_media', '임베드 원본 (squat/stoop/box/gait/carry _ppt.mp4 + figure png)'),
])}

### 10.3 모델

{table([
    row(ROMMODEL, f'**최종 통일 모델**. 기저 해시 `{BASE_HASH}`'),
    row(f'{MODELS}/MaleFullBodyModel_v2.0_OS4_modified_no_coupler_M1scap_armfix.osim',
        'ROM 수정 전 (이력 보존용)'),
    row('/data/romfix_unified/squat_off/model_res_tight.osim',
        f'reserve 포함 실행 모델. 해시 `{RUN_HASH}` — 10회 실행 전부 동일'),
])}

### 10.4 SO 결과 (5동작 × OFF/ON)

{table([row(f'/data/romfix_unified/{k}_{c}', f'{pn.NAME[k]} {c.upper()}')
        for k in pn.ORDER for c in ('off', 'on')]
       + [row('/data/romfix_unified/unified_numbers.json', '지표 원본 (paper_numbers 입력)'),
          row('/data/romfix_unified/gait_redistribution.json', '보행 재분배 분해'),
          row('/data/romfix_unified/comparison.json', '기존 확정본과의 대조'),
          row('/data/romfix_unified/logs', '실행 로그 10건')])}
...
{table([row(MOTS[k], f'{pn.NAME[k]}' + (' — 좌팔 정정본' if k in ('stoop', 'box', 'carry') else ' — 무변경'))
        for k in pn.ORDER])}
...
{table([row(f'{ROOT}/opensim_analysis/thoracolumbar_fb/scripts/viz_knee_fix/{f}', n) for f, n in [
    ('paper_numbers.py', '⭐ **수치 단일 소스**. 표기 규칙 강제, `.sto` 경로표(`STO`) 포함'),
    ('rerun_romfix_all.py', '5동작 OFF/ON 재실행 (`OMP_NUM_THREADS=1` 필수)'),
    ('analyze_romfix.py', '지표 (a)(b)(c) 산출 + 기존 대조 + 회귀 검증'),
    ('make_rom_fixed_model.py', 'ROM 부호 수정 모델 생성 + 검증'),
    ('fix_leftarm_mirror.py', '좌팔 미러 부호 실측 확정 + `.mot` 재생성'),
    ('audit_conditions.py', '해석 조건 전수 감사'),
    ('crosscheck_docs.py', '5개 문서 수치 일관성 자동 대조'),
    ('analyze_gait_redistribution.py', '보행 근육군 재분배 + Figure 7'),
    ('make_paper_figures.py', 'Figure 3–7'),
    ('make_paper_figures12.py', 'Figure 1–2'),
    ('build_paper_docx.py', '논문 docx 생성 (pdf는 `soffice --headless --convert-to pdf`)'),
    ('build_presentation.py', '발표자료 pptx 생성'),
    ('gen_handover.py', '이 문서 생성기'),
]])}

렌더 파이프라인: `{ROOT}/opensim_analysis/thoracolumbar_fb/scripts/viz_knee_fix/render_*.py`
({len([f for f in os.listdir(f'{ROOT}/opensim_analysis/thoracolumbar_fb/scripts/viz_knee_fix') if f.startswith('render_') and f.endswith('.py')])}개).
...
{table([
    row('/data/wearable-assist-assets/blender',
        'Blender 저작 파일 2종(`muskemo_scene.blend`, `pose2sim_scene.blend`). '
        '동봉 `README.md`에 용도·재생성 가능 여부 기재'),
])}

### 10.8 검증 그리드 (GitHub raw 접근 가능)

{table([
    row(f'{DOCS}/images/shoulder_diag/shoulder_diag_verification_grid.png', '좌측 어깨 진단'),
    row(f'{DOCS}/images/shoulder_diag/romfix_rerun_verification_grid.png', 'ROM+좌팔 수정 재실행 검증'),
    row(f'{DOCS}/images/paper_five_motion/paper_final_pages_01_10.png', '논문 1–10쪽 미리보기'),
    row(f'{DOCS}/images/paper_five_motion/paper_final_pages_11_20.png', '논문 11–20쪽 미리보기'),
])}
...
with open(OUT, 'w', encoding='utf-8') as f:
    f.write(md)
print('SAVED', OUT, f'{os.path.getsize(OUT)/1000:.1f} KB')
