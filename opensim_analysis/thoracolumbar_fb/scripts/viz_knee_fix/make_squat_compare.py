...
SRC="/tmp/cmp_render/squat_pub"  # squat deep-flexion t=1.992, AMIN0.13 VMAX0.43
D=json.load(open("/tmp/cmp_render/squat_frames/frame_1.992.json"))
...
OUT=os.path.join(OUTDIR,"squat_deepflex_compare_grid.png")
...
d.text((PAD,16),"쪼그려 앉아 들 때(squat) — 가장 깊이, 허리 근육 부담 비교", fill=(255,255,255), font=F(36,True))
d.text((PAD,64),"무릎·고관절 깊이 굽힘(deep squat). 슈트만 다름. 왼쪽 허리 빨강(힘듦) vs 오른쪽 옅음(편함).", fill=(205,210,220), font=F(20))
def zoom_es(p):
    im=Image.open(p).convert("RGB"); a=np.asarray(im).astype(int); bg=a[2,2]
