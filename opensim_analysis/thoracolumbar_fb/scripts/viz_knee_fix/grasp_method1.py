...
# METHOD 1: palm on side faces, palm toward box center
FRONT=BCX-HALF      # box front face x
BOTY=TOPb           # box bottom (on table top)
TR=np.array([BCX, TOPb+0.10, +HALF])
TL=np.array([BCX, TOPb+0.10, -HALF])
...
def cons():
    ...
    puR=1+handZ('hand_R')[2]; puL=1-handZ('hand_L')[2]
...
json.dump({'pose':{k:float(v) for k,v in pose.items()},'geo':{'edge':EDGE,'top':TOPb,'grip':[BCX,TOPb+0.05,0.0],'half':HALF},'resid':{k:float(v) for k,v in g.items()}}, open('/tmp/cmp_render/m1_pose.json','w'))
print("SAVED m1_pose.json")
