...
FRAME="/tmp/cmp_render/table50n_frame.json"; OSIM="/tmp/cmp_render/tlfb/TLFB.osim"
OUT="/tmp/cmp_render/table50n"; os.makedirs(OUT,exist_ok=True); RAD=0.006
...
def _force_visible(ob):
    for c in list(ob.users_collection):
        try: c.objects.unlink(ob)
        except: pass
    bpy.context.scene.collection.objects.link(ob); ob.hide_render=False
...
# BOX at hands
bpy.ops.mesh.primitive_cube_add(size=BOXSZ, location=(bc[0],bc[1],bc[2])); box=bpy.context.object; box.data.materials.append(smat("box",(0.85,0.55,0.20),1,0.6)); box.name="BOX"; _force_visible(box)
...
tb=bpy.context.object; tb.scale=(0.45,tbl_h/2,0.6); tb.data.materials.append(smat("table",(0.45,0.47,0.55),1,0.1)); tb.name="TABLE"; _force_visible(tb)
