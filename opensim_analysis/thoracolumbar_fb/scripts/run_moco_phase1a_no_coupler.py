"""Phase 1a regression — same setup as run_moco_phase1a_full.py but model = _no_coupler.

Used for Step 2.B.2 regression test (verify coupler removal does not change ES results).

Usage:
  python run_moco_phase1a_no_coupler.py smoke   # t=1.0-3.0, mesh=25 (~5-10 min)
  python run_moco_phase1a_no_coupler.py full    # t=0.0-5.0, mesh=50 (production)
"""
...
SRC_MODEL = '/data/opensim_models/ThoracolumbarFB/Fullbody_TLModels_v2.0_OS4x/MaleFullBodyModel_v2.0_OS4_moco_stoop_no_coupler.osim'
...
def prepare_reference(out_path, t_start, t_end):
    ...
    n = len(keep)
    header = (f"stoop_v5_phase1a_no_coupler\nversion=1\nnRows={n}\n"
...
def run_inverse(model_path, grf_xml, ref_path, t_start, t_end,
    ...
    log('--- MocoInverse setup with GRF (no coupler) ---')
    inverse = osim.MocoInverse()
    inverse.setName('phase1a_no_coupler')
...
def main():
    ...
    if mode == 'smoke':
        ...
        mesh = 25
        out_root = Path('/data/wearable-assist/results/phase1a_smoke_no_coupler')
    elif mode == 'full':
        t_start, t_end = 0.0, 5.0
        mesh = 50
        out_root = Path('/data/wearable-assist/results/phase1a_full_no_coupler')
    else:
    ...
    log(f'=== mode={mode}  no_coupler  t=[{t_start},{t_end}]  mesh={mesh} ===')
