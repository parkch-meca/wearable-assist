"""Phase 1a regression — forearm_v1 model (손 세그먼트 19.2 cm 추가).

Phase 1a no_coupler 결과와 비교하여 forearm 수정이 ES 활성화에 미치는 영향을 검증.

Usage:
  python run_moco_phase1a_forearm_v1.py smoke   # t=1.0-3.0, mesh=25 (~5-10 min)
  python run_moco_phase1a_forearm_v1.py full    # t=0.0-5.0, mesh=50 (production)
"""
...
def prepare_reference(out_path, t_start, t_end):
    ...
    n = len(keep)
    header = (f"stoop_v5_phase1a_forearm_v1\nversion=1\nnRows={n}\n"
...
def run_inverse(model_path, grf_xml, ref_path, t_start, t_end,
    ...
    log('--- MocoInverse setup with GRF (forearm_v1) ---')
    inverse = osim.MocoInverse()
    inverse.setName('phase1a_forearm_v1')
...
def main():
    ...
    if mode == 'smoke':
        ...
        mesh = 25
        out_root = Path('/data/wearable-assist/results/phase1a_smoke_forearm_v1')
    elif mode == 'full':
        t_start, t_end = 0.0, 5.0
        mesh = 50
        out_root = Path('/data/wearable-assist/results/phase1a_full_forearm_v1')
    else:
    ...
    log(f'=== mode={mode}  forearm_v1  t=[{t_start},{t_end}]  mesh={mesh} ===')
